...
# Set either ANDROID_NDK_ROOT or ANDROID_NDK_HOME to you NDK location. 
# For example,
...
if [ -z "$ANDROID_NDK_ROOT" ] || [ ! -d "$ANDROID_NDK_ROOT" ]; then
  echo "Error: ANDROID_NDK_ROOT ($ANDROID_NDK_ROOT) is not a valid path. Please edit this script."
  exit 1
fi
